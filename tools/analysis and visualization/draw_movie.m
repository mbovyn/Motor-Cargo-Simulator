...
switch plot_box
    ...
    case 1 %cargo set at 0 and not moving
        ...
        zends=[-(R+max_length+z_MT_offset+.02) R+max_length];
...
%                     zm*2*R_MT+loc_head(n,3));
    ...
    %MT
    [zc,yc,xc] = cylinder(R_MT);
    h_cyl=surf((4*(xc-.5)),yc+y_MT,zc+z_MT,'edgealpha',0.3);
    set(h_cyl,'FaceColor','g');

    text(xends(2),y_MT,z_MT,'(+)',...
        'FontWeight','bold','FontSize',16 ...
        ,'VerticalAlignment','bottom')
    text(xends(1),y_MT,z_MT,'(-)','FontWeight','bold'...
