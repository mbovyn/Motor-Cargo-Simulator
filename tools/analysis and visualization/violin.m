...
xL=[];
%fc=[0 0 0];
fc=[0 0.4470 0.7410]; %(default blue)
lc='k';
alp=0.5;
mc='k';
medc='r';
...
%added by MB
supp = repmat({'unbounded'},length(Y),1);
...
% edited by MB to take in support argument for ksdensity
if isempty(find(strcmp(varargin,'Support')))==0
    supp = varargin{find(strcmp(varargin,'Support'))+1};
end
%%
if size(fc,1)==1
...
for i=1:size(Y,2)
    ...
    if isempty(b)==0
        [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i),'Support',supp{i});
    elseif isempty(b)
        [f, u, bb]=ksdensity(Y{i},'Kernel','triangle','Support',supp{i});
    end
