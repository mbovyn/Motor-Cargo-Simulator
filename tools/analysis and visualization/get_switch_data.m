...
for i=1:nruns(1)
    for j=1:nruns(2)
        ...
        n_stayed_on=sum(stayed_on{i,j});
        p_ToW(i,j)=sum(towed{i,j} & stayed_on{i,j})/n_stayed_on;
        p_switch(i,j)=sum(switched{i,j} & stayed_on{i,j})/n_stayed_on;
        ...
        SE_ToW(i,j)=sqrt(p_ToW(i,j)*(1-p_ToW(i,j))/n_stayed_on);
        SE_switch(i,j)=sqrt(p_switch(i,j)*(1-p_switch(i,j))/n_stayed_on);
        SE_switchToW(i,j)=sqrt(p_switchToW(i,j)*(1-p_switchToW(i,j))/n_stayed_on);
